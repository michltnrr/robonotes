...
modeBtns.forEach(btn => {
    btn.addEventListener('click', function() {
        ...
        if (currentMode === 'video') {
            ...
            input.placeholder = 'Paste a YT video url for a summarization';
        }
    });
});
...
async function generate() {
    ...
    if(currentMode === 'quiz') {
        ...
        generatedquizData = response.data
       ...
       downloadBtn.style.font = 'Roboto'
       downloadBtn.style.fontSize = 'medium'
       downloadBtn.style.width = '100px'
       downloadBtn.style.height = '30px'
       downloadBtn.style.marginLeft = '100px'
       downloadBtn.style.marginRight = '135px'
       downloadBtn.style.marginTop = '20px'
       downloadBtn.style.color = 'white'
       downloadBtn.style.background =  `linear-gradient(to right, rgb(217, 59, 85), rgb(255, 47, 0))`
       downloadBtn.dataset.path = response.videoPath

       courseId = document.getElementById(`courseSelect`).value

       downloadBtn.addEventListener('click', async () => {
        const videoPath = downloadBtn.dataset.path
        if (!videoPath) return
        ...
        try {
            const savevidmetaData = await fetch(`/courses/${courseId}/notes`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                ...
                body: JSON.stringify({
                    ...
                    prompt: promptAction,
                    metadata: {
                        title: response.videoPath.match(/([^/]+)\.mp4$/)?.[1],
                        path: response.videoPath
                    }
                })
            })
...
saveButton.addEventListener('click', async () => {
    ...
    try {
        if(currentMode === 'summary') {
            saveNote = await fetch(`/courses/${courseId}/notes`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${token}`
                ...
                body: JSON.stringify({
                    ...
                    prompt: promptAction,
                    metadata: {
                        content: outputContent.textContent
                    }
                })
            })
            ...
            saveNote = await fetch(`/courses/${courseId}/notes`, {
                ...
                body: JSON.stringify({
                    ...
                    course: courseName,
                    metadata: {
                        title: generatedquizData.quizTitle,
